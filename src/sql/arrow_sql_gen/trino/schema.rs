...
pub(crate) fn trino_data_type_to_arrow_type(trino_type: &str, tz: Option<&str>) -> Result<DataType> {
    ...
    if normalized_type.contains("with time zone") && normalized_type.starts_with("timestamp(") {
        ...
        return Ok(DataType::Timestamp(time_unit, Some(Arc::from(tz.unwrap_or("UTC")))));
    }
